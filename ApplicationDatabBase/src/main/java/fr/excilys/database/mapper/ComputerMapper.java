...
public class ComputerMapper implements RowMapper<Computer> {
	...
	public Computer mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		return new ComputerBuilder().id(id).name(name).introduced(introduced).discontinued(discontinued)
				.company(company).build();
	}

	public Computer computerDtoToComputer(ComputerDTO computerDto) {
		return new ComputerBuilder().id(computerDto.getId()).name(computerDto.getName())
